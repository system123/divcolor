...
class lab_imageloader_dual:

  def __init__(self, data_directory, out_directory, subdirs=(None, None) \
            listdir=None, shape=(64, 64), ext='JPEG', outshape=(256, 256)):

    if(listdir == None):
      if(subdirs[0] == None):
        self.test_img_fns = glob.glob('%s/*.%s' % (data_directory, ext))
      else:
        self.test_img_fns = glob.glob('%s/%s/*.%s' % (data_directory, subdirs[0], ext))
      self.train_img_fns = []
    else:
      self.train_img_fns = []
      self.test_img_fns = []
      self.train_img_fns_lum = []
      self.test_img_fns_lum = []
      with open('%s/list.train.vae.txt' % listdir, 'r') as ftr:
        for img_fn in ftr:
          self.train_img_fns.append(os.path.join(data_directory, subdirs[0], img_fn.strip('\n')))

          if subdirs[1] is not None:
              self.train_img_fns_lum.append(os.path.join(data_directory, subdirs[1], img_fn.strip('\n')))

      with open('%s/list.test.vae.txt' % listdir, 'r') as fte:
        for img_fn in fte:
          self.test_img_fns.append(os.path.join(data_directory, subdirs[0], img_fn.strip('\n')))

         if subdirs[1] is not None:
             self.test_img_fns_lum.append(os.path.join(data_directory, subdirs[1], img_fn.strip('\n')))
  ...
  def train_next_batch(self, batch_size, nch):
    ...
    for i_n, i in enumerate(range(self.train_batch_head, self.train_batch_head+batch_size)):
      ...
      # Load the luminance channel from the original SAR image.
      img_large_lum = None
      if len(train_img_fns_lum) > 0:
          img_large_lum = cv2.imread(self.train_img_fns_lum[currid])
